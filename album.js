...
async function loadAlbumData(){
  ...
  if(inline){
    ...
    if(txt&&txt.startsWith("{")){try{return JSON.parse(txt);}catch(e){}}
  }
...
const rafThrottle=fn=>{let p=false;return function(...a){if(p)return;p=true;requestAnimationFrame(()=>{p=false;fn.apply(this,a);});}};
const idle=cb=>{if("requestIdleCallback"in window)requestIdleCallback(cb,{timeout:200});else setTimeout(cb,50)};
...
(async function(){
  ...
  let BOOK;
  try{BOOK=await loadAlbumData();}catch(err){console.error(err);alert("Не удалось загрузить данные альбома. Если вы открываете файл локально (file://), вставьте JSON в <script id=\"albumData\"> или подключите album-data.js. На GitHub Pages всё будет работать автоматически.");return;}
  ...
  function formatDateRU(iso){try{const d=new Date(iso+"T12:00:00");return d.toLocaleDateString("ru-RU",{day:"2-digit",month:"long",year:"numeric"});}catch{return iso;}}
  ...
  function preload(src){
    if(!src)return;
    idle(()=>{const im=new Image();im.loading="eager";im.decoding="async";im.src=src;});
  }
  ...
  autoplayBtn.addEventListener("click",()=>{playing=!playing;autoplayBtn.setAttribute("aria-pressed",playing?"true":"false");if(playing){autoTimer=setInterval(()=>{curlFX("right");loveSparks("right");next("right");},3600);}else{stopAuto();}},{passive:true});
  ...
  updateScrollerVisibility();addEventListener("resize",rafThrottle(updateScrollerVisibility),{passive:true});
